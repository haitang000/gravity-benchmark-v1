...
def retry_kind(result: GenerationResult, value: float, details: dict[str, Any]) -> str | None:
    ...
    if value < 1 and details.get("kind") != "sandbox_unavailable": return "score"
    return None
            ...
            for repeat in range(1, repeats + 1):
                async def one(task, repeat):
                    ...
                    async with sem:
                        ...
                        attempts = error_retries = score_retries_used = 0
                        while True:
                            attempts += 1
                            try:
                                result = await adapter.generate([Message(role="user", content=task.prompt)], generation, on_text)
                            ...
                            kind = retry_kind(result, value, details)
                            if kind == "error" and error_retries < retries: error_retries += 1; continue
                            if kind == "score" and score_retries_used < score_retries: score_retries_used += 1; continue
                            break
                        streams.get(run_id, {}).pop(key, None)
                        metrics = result.model_dump(exclude={"text", "error", "backend_metadata"}) | result.backend_metadata | {"attempts": attempts, "error_retries": error_retries, "score_retries": score_retries_used, "repeat": repeat}
                ...
                await asyncio.gather(*(one(task, repeat) for task in tasks))
            await adapter.close()
