...
from backend.schemas import GenerationConfig, RunStatus, Message
from backend.storage.database import TaskResult, add_result, get_profile, get_run, update_run

jobs: dict[int, asyncio.Task] = {}

async def execute(run_id: int, model_ids: list[int], suite: str, languages: list[str], generation: GenerationConfig, counts: dict[str,int], seed: int, concurrency: int) -> None:
    run = get_run(run_id); tasks = load_tasks(suite, languages, seed, counts); update_run(run_id, status=RunStatus.RUNNING, total_tasks=len(tasks) * len(model_ids), settings={"model_ids": model_ids, "suite": suite, "languages": languages, "generation": generation.model_dump(), "seed": seed, "dataset_hash": dataset_hash()})
        ...
        for model_id in model_ids:
            ...
            async def one(task):
                ...
                async with sem:
                    ...
                    metrics = result.model_dump(exclude={"text", "error", "backend_metadata"}) | result.backend_metadata
