...
def aggregate(results: list[Any]) -> dict[str, Any]:
    ...
    def grouped(key: str) -> dict[str, Any]:
        ...
        return {name: _token_summary(rows) for name, rows in buckets.items()}

    def _token_summary(rows: list[Any]) -> dict[str, Any]:
        inputs = [r.metrics.get("input_tokens") for r in rows if r.metrics.get("input_tokens") is not None]
        outputs = [r.metrics.get("output_tokens") for r in rows if r.metrics.get("output_tokens") is not None]
        totals = [total for r in rows if (total := token_total(r.metrics)) is not None]
        return {"count": len(rows), "score": mean([r.score for r in rows]) if rows else 0, "pass_rate": sum(r.passed for r in rows) / len(rows) if rows else 0, "avg_input_tokens": mean(inputs) if inputs else None, "avg_output_tokens": mean(outputs) if outputs else None, "avg_total_tokens": mean(totals) if totals else None}
